...
void printRootDirElements(){
    ...
    for(; i<31;i++){
        ...
        fprintf(stderr, "%d  file : %s  Inode Num %d size %d \n", i, rootDir.table[i].fileName, rootDir.table[i].inodeNumber,inodeTable[i].fileSize );
...
void printFirstTenINodes(){
    ...
    for(; i<10;i++){
        if(inodeBitmap[i]=='1'){
            fprintf(stderr, "Inode Num %d size %d \n", inodeTable[i].inode_number,inodeTable[i].fileSize );
        }
    }
    fprintf(stderr, "}\n");
}
...
int sfs_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    */
    ...
    do{
        ...
        if(sizeBetween >= bytesRemaining){ //last call
           
            if((sizeBetween + bytesRead)>=NODE->fileSize){
                //only read the size in
                int diff = NODE->fileSize - bytesRead;
                bytesRead += diff;
                pread(diskfile, buf+buffOffset, diff, (722+currDiskBlock)*BLOCK_SIZE);
            ...
            return bytesRead;
        }else if (sizeBetween < bytesRemaining){//more call
            ...
            if((sizeBetween + bytesRead)>=NODE->fileSize){
                ...
                pread(diskfile, buf+buffOffset, diff, (722+currDiskBlock)*BLOCK_SIZE);
                fprintf(stderr, "0) returning %d bytes read  \n", bytesRead);
    ...
    }while(bytesRemaining > 0 && bytesRead < NODE->fileSize);
